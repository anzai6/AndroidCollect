...
//    private ScrollView scrollView;
    ...
    int offset = OFF_SET_DEFAULT; // 偏移量（�?要在�?前面和最后面补全�?
...
//                    Logger.d(TAG, "remainder: " + remainder + ", divided: " + divided);
    ...
    private TextView createView(String item) {
        ...
        int padding = DpPxUtils.dip2px(context, 15);
...
//            position = divided + offset + 1;
    ...
    public void setBackgroundDrawable(Drawable background) {
        ...
        if (null == paint) {
            ...
            paint.setStrokeWidth(DpPxUtils.dip2px(context, 1f));
        }
    ...
    private void onSeletedCallBack() {
        if (null != onWheelViewListener) {
            if (selectedIndex >= 0 && selectedIndex <= items.size() - 1) {
                onWheelViewListener.onSelected(selectedIndex - 1, items.get(selectedIndex));
            }
        }
